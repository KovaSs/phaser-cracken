...
3. **Проверка Go-бинарника (запуск сервера)** — Go-бинарник хранит таймстемп первой ошибки авторизации в `server.log`. По истечении 96-часового грейс-периода он блокирует запуск. Прокси очищает `server.log` и `auth-failure-v1.log` при каждом запуске, давая новый грейс-период.
