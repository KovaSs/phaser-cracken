...
Παρακάμπτονται τρία επίπεδα προστασίας:
...
3. **Έλεγχος Go δυαδικού (εκκίνηση διακομιστή)** — το Go δυαδικό αποθηκεύει τη χρονική σήμανση αποτυχίας αυθεντικοποίησης στο `server.log`. Όταν λήξει η περίοδος χάριτος των 96 ωρών, αρνείται να ξεκινήσει. Το proxy πλέον μηδενίζει τα `server.log` και `auth-failure-v1.log` σε κάθε κλήση, δίνοντας μια νέα περίοδο χάριτος κάθε φορά που ξεκινά ο επεξεργαστής.
