...
Три уровня защиты обходятся:

1. **Проверка Electron JS** — патчит `WindowManager.js`, заставляя `isEditorActivated()` всегда возвращать `true`.
2. **Прокси для Go-бинарника** — устанавливает прозрачный прокси вокруг `PhaserEditor`, перехватывающий вызов `-tool print-user-status` и возвращающий фиктивный ответ о подписке. Все остальные команды передаются реальному бинарнику.
3. **Сброс grace-периода** — Go-бинарник хранит таймстемп первой ошибки авторизации в `server.log`. По истечении 96-часового grace-периода он блокирует запуск. Прокси очищает `server.log` и `auth-failure-v1.log` при каждом запуске, давая новый grace-период. Пакетный файл сессии (`copy-session`) предотвращает пропуск валидации, когда сессия отсутствует.
...
npm run phaser-cracken --copy-session     # Установить встроенный файл сессии
...
```bash
#!/bin/bash
# Сбрасывает грейс-период, перехватывает print-user-status, всё остальное передаёт дальше
...
| `copy-session [source]`  | Установить файл сессии (по умолчанию использует встроенный ресурс или собственный путь)        |
...
| `auto`                   | Полная настройка: патч + прокси + copy-session + сброс грейса + запуск                          |
...
| `resources/user-session-v3.bin`          | Встроенный файл сессии              |
...
### Слой 3: Период grace-периода и файл сессии

Без файла `user-session-v3.bin` Go-бинарник полностью пропускает HTTP-проверку и сразу переходит к ошибке "premium users". Пакетный файл сессии находится в `resources/` — `copy-session` устанавливает его в `~/.phasereditor2d/`.
